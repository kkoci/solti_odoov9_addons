...
class invoice(models.Model):
    ...
    def send_xml_file(self):
        ...
        post = '/DTEWS/services/wsDTECorreo?wsdl' # HTTP/1.1
        url = host + post
        ...
        '''if isinstance(self.sii_xml_request, bool):
            content_length == int(self.sii_xml_request)  # 1 if True else 0
        else:
        ...
        response = pool.urlopen('POST', url, headers={
            ...
            'charset': 'utf-8',
            'Content-Length': (len(
                self.sii_xml_request) if self.sii_xml_request is not True or False else '')}, body=self.sii_xml_request) 
